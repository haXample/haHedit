...
_KT3    DD      _SIZE_TABLE DUP (?)
_KT2    DD      _SIZE_TABLE DUP (?)
_KT1    DD      _SIZE_TABLE DUP (?)
_KT0    DD      _SIZE_TABLE DUP (?)

_FT3    DD      _SIZE_TABLE DUP (?)
_FT2    DD      _SIZE_TABLE DUP (?)
_FT1    DD      _SIZE_TABLE DUP (?)
_FT0    DD      _SIZE_TABLE DUP (?)

_RT0    DD      _SIZE_TABLE DUP (?)
_RT1    DD      _SIZE_TABLE DUP (?)
_RT2    DD      _SIZE_TABLE DUP (?)
_RT3    DD      _SIZE_TABLE DUP (?)

_FSb    DB      _SIZE_SBOX DUP (?)
_RSb    DB      _SIZE_SBOX DUP (?)

_RCON   DD      _SIZE_ROUND_CONST DUP (?)
_BSS    ENDS
...
aesGenTab_1:                                          ; Loop #1      ; Loop #2      ; Loop #3
        ...
        mov      BYTE PTR _pow[ecx], al ; pow[i] = x;   Pow[0]=0x01    Pow[1]=0x03    Pow[2]=0x05
        mov      BYTE PTR _log[eax], cl ; log[x] = i;   log[1]=0x00    log[3]=0x01    log[5]=0x02
...
aesGenTab_2:                                            ; Loop #0     ; Loop #7    ; Loop #8    ; Loop #9                          
        movzx    eax, BYTE PTR __x                      ; ---------   ; ---------  ; ---------  ; ---------
        shl      eax, 24                                ;             ;            ;            ;
        mov      DWORD PTR _RCON[ecx*SIZEOF DWORD], eax ; 01000000h   ; 80000000h  ; 1B000000h  ; 36000000h
...
aesGenTab_3:                                                            
        ...
        movzx    eax, BYTE PTR _log[ecx]
        mov      ecx, 255
        sub      ecx, eax
        movzx    eax, BYTE PTR _pow[ecx]
...
@@:     ; x ^= y; y = ( y << 1 ) | ( y >> 7 );
        ...
        mov      al, BYTE PTR __x
        mov      ecx, DWORD PTR __i
        mov      BYTE PTR _FSb[ecx], al        ; Forward S-box

        ; RSb[x] = i;
        mov      al, BYTE PTR __i
        movzx    ecx, BYTE PTR __x
        mov      BYTE PTR _RSb[ecx], al        ; Reverse S-box
...
aesGenTab_4:
        ...
        mov      DWORD PTR _FT0[ecx*SIZEOF DWORD], edx  ; FT0[i]
...
aesGenTab_8:
        ...
        xor      eax, edx
        mov      ecx, DWORD PTR __i                      ; ecx = i
        mov      DWORD PTR _RT0[ecx*SIZEOF DWORD], eax   ; RT0[i]
...
;            to gain [ebp] as an additional multipurpose register) 
;
aesSetKey PROC C USES ebx esi edi, _ctx:DWORD, _key:DWORD, _keysize:DWORD
        ...
        xor      ecx, ecx                       ; Init index counter __i=0

aesSetKey_1:                                
        ; case 128: ctx->nr = 10; break;
        mov      DWORD PTR [edx]._ctxStruct.rnr, 10
        cmp      eax, 128                       ; Keysize 128 bits
        je       SHORT @F
        
        ; case 192: ctx->nr = 12; break;
        mov      DWORD PTR [edx]._ctxStruct.rnr, 12
        cmp      eax, 192                       ; Keysize 192 bits
        je       SHORT @F
        
        ; case 256: ctx->nr = 14; break;
        mov      DWORD PTR [edx]._ctxStruct.rnr, 14
        cmp      eax, 256                       ; Keysize 256 bits
        jne      aesSetKey_ret                  ; default : return; break;
        
@@:     ; for( i = 0; i < (keysize >> 5); i++ )
        ; GET_UINT32_BE( RK[i], key, i << 2 );
        mov      edx, DWORD PTR _key
        mov      eax, DWORD PTR [edx+ecx*SIZEOF DWORD]    ; ecx = i
        bswap    eax
...
;       -----------------
        ...
        cmp      DWORD PTR [edx]._ctxStruct.rnr, 14 ; 256bit-Key: 14 rounds
        je       aesSetKey_4

        cmp      DWORD PTR [edx]._ctxStruct.rnr, 12 ; 192bit-Key: 12 rounds
        je       aesSetKey_3

        cmp      DWORD PTR [edx]._ctxStruct.rnr, 10 ; 128bit-Key: 10 rounds
...
aesSetKey_2:
        ...
        ; RK[5]  = RK[1] ^ RK[4];
        mov      eax, DWORD PTR [ecx+(1*SIZEOF DWORD)]    ; eax = RK[1]
        xor      eax, DWORD PTR [ecx+(4*SIZEOF DWORD)]
        mov      DWORD PTR [ecx+(5*SIZEOF DWORD)], eax    ; RK[5] = eax

        ; RK[6]  = RK[2] ^ RK[5];
        mov      eax, DWORD PTR [ecx+(2*SIZEOF DWORD)]    ; eax = RK[2]
        xor      eax, DWORD PTR [ecx+(5*SIZEOF DWORD)]
        mov      DWORD PTR [ecx+(6*SIZEOF DWORD)], eax    ; RK[6] = eax

        ; RK[7]  = RK[3] ^ RK[6];
        mov      eax, DWORD PTR [ecx+(3*SIZEOF DWORD)]    ; eax = RK[3]
        xor      eax, DWORD PTR [ecx+(6*SIZEOF DWORD)]
        mov      DWORD PTR [ecx+(7*SIZEOF DWORD)], eax    ; RK[7] = eax

        add      ecx, 4*SIZEOF DWORD                      ; RK += 4;
        inc      DWORD PTR __i                            ; i++;
...
aesSetKey_3:
        ...
        ; RK[8]  = RK[2] ^ RK[7];
        mov      eax, DWORD PTR [ecx+2*SIZEOF DWORD]      ; eax = RK[2]
        xor      eax, DWORD PTR [ecx+7*SIZEOF DWORD]
        mov      DWORD PTR [ecx+8*SIZEOF DWORD], eax      ; RK[8] = eax

        ; RK[9]  = RK[3] ^ RK[8];
        mov      eax, DWORD PTR [ecx+3*SIZEOF DWORD]      ; eax = RK[3]
        xor      eax, DWORD PTR [ecx+8*SIZEOF DWORD]
        mov      DWORD PTR [ecx+9*SIZEOF DWORD], eax      ; RK[9] = eax

        ; RK[10] = RK[4] ^ RK[9];
        mov      eax, DWORD PTR [ecx+4*SIZEOF DWORD]      ; eax = RK[4]
        xor      eax, DWORD PTR [ecx+9*SIZEOF DWORD]
        mov      DWORD PTR [ecx+10*SIZEOF DWORD], eax     ; RK[10] = eax

        ; RK[11] = RK[5] ^ RK[10];
        mov      eax, DWORD PTR [ecx+5*SIZEOF DWORD]      ; eax = RK[5]
        xor      eax, DWORD PTR [ecx+10*SIZEOF DWORD]
        mov      DWORD PTR [ecx+11*SIZEOF DWORD], eax     ; RK[11] = eax

        add      ecx, 6*SIZEOF DWORD                      ; RK += 6;
        inc      DWORD PTR __i                            ; i++;
...
aesSetKey_4:                                              ; Keysize 256 bits
        ...
        ;     ( FSb[ (uint8) ( RK[11] >>  8 ) ] <<  8 ) ^
        ;     ( FSb[ (uint8) ( RK[11]       ) ]       );
        mov      edx, DWORD PTR [ecx+11*SIZEOF DWORD]     ; RK[11]
        shr      edx, 24
        movzx    edx, dl
        movzx    eax, BYTE PTR _FSb[edx]
        shl      eax, 24
        xor      eax, DWORD PTR [ecx+4*SIZEOF DWORD]      ; RK[4]
        mov      edx, DWORD PTR [ecx+11*SIZEOF DWORD]     ; RK[11]
        shr      edx, 16
        movzx    edx, dl
        ...
        shl      edx, 8
        xor      eax, edx
        movzx    edx, BYTE PTR [ecx+11*SIZEOF DWORD]      ; RK[11]
        movzx    edx, BYTE PTR _FSb[edx]
        xor      eax, edx
        mov      DWORD PTR [ecx+12*SIZEOF DWORD], eax     ; RK[12]

        ; RK[13] = RK[5] ^ RK[12];
        mov      eax, DWORD PTR [ecx+5*SIZEOF DWORD]      ; eax = RK[5]
        xor      eax, DWORD PTR [ecx+12*SIZEOF DWORD]
        mov      DWORD PTR [ecx+13*SIZEOF DWORD], eax     ; RK[13] = eax

        ; RK[14] = RK[6] ^ RK[13];
        mov      eax, DWORD PTR [ecx+6*SIZEOF DWORD]      ; eax = RK[6]
        xor      eax, DWORD PTR [ecx+13*SIZEOF DWORD]
        mov      DWORD PTR [ecx+14*SIZEOF DWORD], eax     ; RK[14] = eax

        ; RK[15] = RK[7] ^ RK[14];
        mov      eax, DWORD PTR [ecx+7*SIZEOF DWORD]      ; eax = RK[7]
        xor      eax, DWORD PTR [ecx+14*SIZEOF DWORD]     
        mov      DWORD PTR [ecx+15*SIZEOF DWORD], eax     ; RK[15] = eax

        add      ecx, 8*SIZEOF DWORD                      ; RK += 8;
        inc      DWORD PTR __i                            ; i++;
...
aesSetKey_6:
        movzx    ebx, BYTE PTR _FSb[edx]                  ; ebx = FSb[i]

        ; KT0[i] = RT0[ FSb[i] ];
        mov      eax, DWORD PTR _RT0[ebx*SIZEOF DWORD]    
        mov      DWORD PTR _KT0[edx*SIZEOF DWORD], eax
        ; KT1[i] = RT1[ FSb[i] ];
        mov      eax, DWORD PTR _RT1[ebx*SIZEOF DWORD]
        mov      DWORD PTR _KT1[edx*SIZEOF DWORD], eax
        ; KT2[i] = RT2[ FSb[i] ];
        mov      eax, DWORD PTR _RT2[ebx*SIZEOF DWORD]
        mov      DWORD PTR _KT2[edx*SIZEOF DWORD], eax
        ; KT3[i] = RT3[ FSb[i] ];
        mov      eax, DWORD PTR _RT3[ebx*SIZEOF DWORD]
        mov      DWORD PTR _KT3[edx*SIZEOF DWORD], eax
 
        inc      edx
        cmp      edx, _SIZE_TABLE                         ; 256
        jb       aesSetKey_6
;       --------------------
...
aesSetKey_7:
        ...
        add      edx, _ctxStruct.drk                      ; Decryption round keys 
        mov      DWORD PTR _SK, edx                       ; edx = SK
                                                          ; ecx = RK
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+0*SIZEOF DWORD]      ; RK[0]
        mov      DWORD PTR [edx+0*SIZEOF DWORD], eax      ; SK[0]
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+1*SIZEOF DWORD]      ; RK[1]
        mov      DWORD PTR [edx+1*SIZEOF DWORD], eax      ; SK[1]
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+2*SIZEOF DWORD]      ; RK[2]
        mov      DWORD PTR [edx+2*SIZEOF DWORD], eax      ; SK[2]
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+3*SIZEOF DWORD]      ; RK[3]
        mov      DWORD PTR [edx+3*SIZEOF DWORD], eax      ; SK[3]

        ; for( i = 1; i < ctx->nr; i++ )
        mov      DWORD PTR __i, 1

aesSetKey_8:
        sub      ecx, 4*SIZEOF DWORD                      ; RK -= 4;
        add      edx, 4*SIZEOF DWORD                      ; SK += 4;
        ...
        cmp      eax, DWORD PTR [ebx]._ctxStruct.rkEnd    ; +512
        jb       aesSetKey_8
;       --------------------
        
aesSetKey_exit:
        sub      ecx, 4*SIZEOF DWORD                      ; RK -= 4;            
        add      edx, 4*SIZEOF DWORD                      ; SK += 4; (edx = ctx.drk)
                                                          
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+0*SIZEOF DWORD]      ; eax = RK[0]
        mov      DWORD PTR [edx+0*SIZEOF DWORD], eax      ; SK[0] = eax
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+1*SIZEOF DWORD]      ; eax = RK[1]
        mov      DWORD PTR [edx+1*SIZEOF DWORD], eax      ; SK[1] = eax
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+2*SIZEOF DWORD]      ; eax = RK[2]
        mov      DWORD PTR [edx+2*SIZEOF DWORD], eax      ; SK[2] = eax
        ; *SK++ = *RK++;
        mov      eax, DWORD PTR [ecx+3*SIZEOF DWORD]      ; eax = RK[3]
        mov      DWORD PTR [edx+3*SIZEOF DWORD], eax      ; SK[3] = eax
...
;
_aesAlgorithm PROC C USES ebx esi edi, _ctx, _input, _output
        ...
        cmp      DWORD PTR [ecx]._ctxStruct.rnr, 10     ; 128bit-key?   
        ...
        cmp      DWORD PTR [edx]._ctxStruct.rnr, 12     ; 192bit-key?   
...
;
aesAlgorithm PROC C PUBLIC USES ebx esi edi, _inblock:DWORD, _outblock:DWORD, _mode:DWORD
;
...
;
aesKeyInit PROC C PUBLIC USES ebx esi edi, _key:DWORD, _keylen:DWORD, _mode:DWORD
